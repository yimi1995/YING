...
class _LSTMModel(ts_model.SequentialTimeSeriesModel):
  ...
  def _filtering_step(self, current_times, current_values, state, predictions):
    ...
    with tf.control_dependencies(
      ...
      #用均方误差表示预测的loss，目前是修改了这一段

      predictions["loss"] = tf.reduce_mean(
              (prediction - transformed_values) ** 2, axis=-1)
      # tf.summary.scalar('loss', predictions["loss"])
...
if __name__ == '__main__':
  ...
  csv_file_name = path.join("./data/ID01_train.csv")
  ...
  #每次预测10步，(predictions记录了每次预测的时间序号和预测的平均值)
  ...
  # writer = tf.summary.FileWriter("D:\software_study\pycharm\project\predict_load\log", tf.get_default_graph())
